...
uint8_t *ArrayRead;										//读存储部件
uint32_t NowAdress;										//最后一个写入数据的地址
uint8_t num;													//W25Q64数据的第几个
...
/*读取所有的信息--可行*/
	...
	W25Q64_ReadData(0x000000,ArrayRead,(uint8_t)NowAdress % 0xffffff);
	/*测试*/
	...
	while (1)
	{
		...
		OLED_ShowNum(2,4,ArrayRead[num],2);
